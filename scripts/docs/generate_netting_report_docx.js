...
const mdPath = path.join(docsDir, "netting-implementation-report.md");
const docxPath = path.join(docsDir, "netting-implementation-report.docx");

const baseCommit = "a5635e7";
const headCommit = "9f9751a";
const branchName = "feat/block-netting";
...
const summaryParagraphs = [
  ...
  `对比基线为原始仓库在 netting 分支开始前的提交 ${baseCommit}；当前实现提交为 ${headCommit}，分支为 ${branchName}。总体差异约为 97 个文件，新增约 11800 行，删除约 69 行。`,
...
const implementationFlow = [
  "用户交易输入后，Supervisor 将跨分片 NormalTx 转换为 PaymentIntent，并发送到源分片。",
  "源分片在普通区块共识过程中执行 intent 预留，写入 registry，并在区块 finalized 后向 Solver 发布 receipt。",
  "Solver 根据 receipt 维护异步 Vector-Cut 窗口，满足 BatchSize 或 MaxWindowDuration 后冻结窗口。",
  "Solver 对窗口内 intent 执行 exact → best-fit → split 撮合，生成 batch sidecar、settlement package 和 MatchRoot。",
  "Beacon 节点验证 batch 可由 receipt 和 matcher mode 确定性派生，再通过 PBFT commit MatchRoot。",
  "普通分片收到 MatchRootFinalized 和 SettlementPackage 后，本地验证 Merkle proof，执行 reserved balance 消耗和收款人入账。",
  "未净额化的剩余金额进入 ReservedFallback 路径，继续走保留资金驱动的普通跨分片回退流程。",
  "Supervisor 与 netting metric collector 收集 batch、intent、Beacon、execution 指标，供实验脚本汇总并画图。",
];
...
function heading(text, level) {
  return new Paragraph({
    ...
    children: [new TextRun({ text, bold: true, size: level === 1 ? 32 : level === 2 ? 27 : 24, font: "Arial" })],
  });
}
...
function table(headers, rows, widths) {
  ...
  const mkCell = (text, width, header = false) =>
    new TableCell({
      borders,
      ...
      children: [
        new Paragraph({
          spacing: { after: 0 },
          children: [new TextRun({ text, bold: header, size: header ? 20 : 18, font: "Arial" })],
        }),
      ],
    });
  return new Table({
    ...
    rows: [
      ...
      ...rows.map((row) => new TableRow({ children: row.map((c, i) => mkCell(String(c), widths[i], false)) })),
    ],
  });
}
...
function docChildren() {
  ...
  children.push(
    new Paragraph({
      ...
      children: [new TextRun({ text: "BlockEmulator-X Netting 论文实现实验文档", bold: true, size: 36, font: "Arial" })],
    }),
  );
  children.push(para("支付意图、资金预留、批量撮合、Beacon MatchRoot 与分片内清算实现说明", { alignment: AlignmentType.CENTER, size: 22 }));
  children.push(para(`生成日期：2026-09-07；仓库：${repoRoot}`, { alignment: AlignmentType.CENTER, size: 18 }));

  children.push(heading("1. 实现总览", 1));
  summaryParagraphs.forEach((p) => children.push(para(p)));
  implementationFlow.forEach((p) => children.push(numbered(p)));

  children.push(heading("2. 分阶段实现路线", 1));
  children.push(table(["阶段", "主题", "主要文件", "作用"], phaseRows, [650, 1900, 3600, 3210]));

  children.push(heading("3. 相对原始 BlockEmulator-X 的文件修改清单", 1));
  children.push(para("下面按功能模块列出核心修改。测试文件没有逐一展开到每个 case，但在最后一行集中说明。"));
  children.push(table(["模块", "涉及文件", "做了什么", "目的"], fileRows, [1200, 2700, 2900, 2560]));

  children.push(heading("4. 7 组论文实验代码与图", 1));
  children.push(para("实验运行脚本为 scripts/netting_experiments/run_experiments.py，画图脚本为 scripts/netting_experiments/plot_experiments.py。plot 脚本输出 figures/netting/*.svg，采用矢量 SVG、论文风格字体、色盲友好配色和简洁网格。"));
  children.push(table(["实验", "自变量/方法", "指标", "论文中回答的问题"], experimentRows, [1300, 2600, 2500, 2960]));

  children.push(heading("5. 验证记录", 1));
  children.push(table(["验证项", "命令", "结果/说明"], validationRows, [1500, 3900, 3960]));

  children.push(heading("6. 推荐代码阅读顺序", 1));
  children.push(para("建议不要从 PBFT 主循环直接硬啃。更顺的方式是先读协议数据结构，再读源分片、Solver、Beacon、分片执行和实验脚本。"));
  children.push(table(["顺序", "阅读目标", "文件", "读懂什么"], readOrderRows, [650, 1900, 3900, 2910]));

  children.push(heading("7. 进一步写论文时可强调的实现边界", 1));
  [
...
function markdown() {
  const lines = [];
  lines.push("# BlockEmulator-X Netting 论文实现实验文档", "");
  lines.push("支付意图、资金预留、批量撮合、Beacon MatchRoot 与分片内清算实现说明", "");
  lines.push(`生成日期：2026-09-07  `);
  lines.push(`仓库：\`${repoRoot}\`  `);
  lines.push(`分支：\`${branchName}\`；基线提交：\`${baseCommit}\`；当前提交：\`${headCommit}\``, "");
  lines.push("## 1. 实现总览", "");
  summaryParagraphs.forEach((p) => lines.push(p, ""));
  implementationFlow.forEach((p, i) => lines.push(`${i + 1}. ${p}`));
  lines.push("");
  addTable(lines, ["阶段", "主题", "主要文件", "作用"], phaseRows, "## 2. 分阶段实现路线");
  addTable(lines, ["模块", "涉及文件", "做了什么", "目的"], fileRows, "## 3. 相对原始 BlockEmulator-X 的文件修改清单");
  addTable(lines, ["实验", "自变量/方法", "指标", "论文中回答的问题"], experimentRows, "## 4. 7 组论文实验代码与图");
  addTable(lines, ["验证项", "命令", "结果/说明"], validationRows, "## 5. 验证记录");
  addTable(lines, ["顺序", "阅读目标", "文件", "读懂什么"], readOrderRows, "## 6. 推荐代码阅读顺序");
  lines.push("## 7. 进一步写论文时可强调的实现边界", "");
  [
    "当前实现先不考虑用户签名和拜占庭求解器安全实验；PaymentIntent 的签名字段和恶意 Solver 检测可作为后续安全性实验扩展。",
    "当前窗口关闭规则保留 BatchSize 与 MaxWindowDuration，不依赖全局区块高度；Vector-Cut 由各分片连续 finalized receipt 冻结，因此可用于异步分片网络。",
    "第 2–7 组实验主要使用受控 synthetic workload，以隔离变量；第 1 组 baseline 使用 selectedTxs_300K.csv 的真实 trace 切片。",
...
function addTable(lines, headers, rows, title) {
  lines.push(title, "");
  lines.push(`| ${headers.join(" | ")} |`);
  lines.push(`| ${headers.map(() => "---").join(" | ")} |`);
  rows.forEach((row) => lines.push(`| ${row.map(mdEscape).join(" | ")} |`));
  lines.push("");
}
